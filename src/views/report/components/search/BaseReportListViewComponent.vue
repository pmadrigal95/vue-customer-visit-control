...
const getVisitControlSearch = async () => {
  try {
    loading.value = true;
    const response = await useVisitControl().visitControlSearch({
      query: {
        customerName: props.entity.companyName, initialDate: props.entity.inititalDate, finalDate: props.entity.endDate,
      }
    });
...
const setReport = (id) => {
  props.entity.reportId = id;
  props.entity.step = 1;
};

onMounted(() => {
  getVisitControlSearch();
});
</script>
        ...
        <div class="flex flex-col justify-center divide-y divide-slate-200 [&>*]:py-16">
          ...
          <div class="w-full">
            ...
            <div class="-my-6">
              ...
              <!-- Item #1 -->
              <div class="relative pl-8 sm:pl-32 py-6 group" v-for="item in list.data" :key="item.id">
                ...
                <a @click="setReport(item.id)"
                  class="cursor-pointer mt-2 inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 border border-gray-200 rounded-lg hover:bg-gray-100 hover:text-blue900 focus:z-10 focus:ring-4 focus:outline-none focus:ring-orange900 focus:text-blue900">
                  Ver más <svg class="w-3 h-3 ml-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 14 10">
