...
defineProps({
 ...
 msg: {
  type: String,
  default: "Hmm, parece que esa página no existe.",
 },
});
...
<template>
    <section class="permission_denied w-full h-full">
        <vue-particles id="tsparticles" :particlesInit="particlesInit" :particlesLoaded="particlesLoaded"
            :options="options" />
        <div class="denied__wrapper w-full h-full pt-44">
            <h1>{{ title }}</h1>
            <h3>{{ msg }}</h3>
            ...
            <a href="#"><button class="denied__link">Regresar</button></a>
        </div>
    </section>
</template>
